/* A mutation performed through a postcondition's result binding is observable
   ...
   The position this encodes: a predicate we evaluate is evaluated
   ...
   GCC gets every shape below right.  This is a
   MIRROR of clang/test/Contracts/Runnable/contract-result-binding-mutation.cpp
   and of its companion ...-mutation-class.cpp, where an epilogue that
   materialises the return value before running the postconditions returns the
   stale copy.  Clang observes
   the scalar shapes; the CLASS-typed ones are lost there and are pinned
   XFAIL on that side.  Nothing else pins the GCC side -- contract-result-
   binding-identity.C covers *which object* the binding names, not whether a
   write through it survives -- so a regression here is silent.
...
/* A class-typed result with a NON-TRIVIAL copy constructor and destructor
   ...
   introduce one for a TRIVIALLY-copyable class: the
