...
//
// Regression: __combined_label::handle_contract_violation must ask whether the
// component's return type is void BEFORE naming the result.  Writing
// `auto __r = _M_lhs.handle_contract_violation(__v);' and testing
// decltype(__r) afterwards is too late -- deducing void for a variable is
// already ill-formed, so combining any void-returning handler fails to compile
// with "deduced type 'void' for '__r' is incomplete".
