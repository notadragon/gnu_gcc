...
/* The UBSan half comes from contracts-routed-checks.def, which is also what
   the wire descriptor and the assume-realization list are built from.  Written
   out by hand instead, a check omitted here while present there would route
   nothing, silently: the descriptor byte stays stock and no diagnostic says
   so.  The ASan and TSan bits are listed directly -- they are not UBSan
   checks and have no routed check id.  */
static const sanitize_code_type routed_ubsan_sanitizer_bits =
#define DEF_ROUTED_UBSAN_CHECK(ID, BIT) BIT |
#include "contracts-routed-checks.def"
  0;

#define ROUTED_SANITIZER_BITS \
  (SANITIZE_USER_ADDRESS | SANITIZE_POINTER_COMPARE \
   | SANITIZE_POINTER_SUBTRACT | SANITIZE_THREAD \
   | routed_ubsan_sanitizer_bits)
