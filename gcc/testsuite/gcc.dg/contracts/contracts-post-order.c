/* A no-result _Post is evaluated at the return, AFTER the return value
   has been computed (D4299).  Emitting the check before the
   return expression instead makes a postcondition observing state mutated
   while computing the return value see the pre-computation state.  */
/* { dg-do run } */
