// P3100: end-to-end default-handler test tying the routing and reporting legs together.
// There is deliberately NO user-defined handle_contract_violation here, so the
// library's own default handler (contract26.cc) runs.  The routing work wired that default
// handler to call contract_violation::report() and print it before its usual
// "contract violation in function ..." summary line; the ASan routed
