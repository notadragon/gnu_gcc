/* A _Post(r: ...) must evaluate the return expression exactly once and
   check the value that is actually returned (D4299).  Emitting the
   return expression twice has the postcondition check the
   first evaluation while a distinct second evaluation is returned.  */
/* { dg-do run } */
