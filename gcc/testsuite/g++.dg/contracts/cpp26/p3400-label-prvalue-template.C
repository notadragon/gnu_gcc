/* A P3400 facet label written as a prvalue -- pre<L{}> -- inside a TEMPLATE
   ...
   inside a template -- is still in template form.  Two ICEs are reachable
   from there:

     * a non-dependent prvalue label is an undigested COMPOUND_LITERAL_P
       CONSTRUCTOR, so store_init_value asserts (cp/typeck2.cc);
     * a label with a dependent subtree constant-evaluated with template
       trees still in it makes dependent_type_p assert (cp/pt.cc).
