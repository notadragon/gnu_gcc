/* A P3400 facet whose result comes from the label's own state -- a data
   ...
   that constant has to be const, TREE_READONLY and
   DECL_DECLARED_CONSTEXPR_P as well as static.  A merely static one cannot
   be read by constant evaluation, so any facet consumed after
   materialization whose result depends on the object's state fails to fold
   -- and fails silently: compute_semantic_core sees error_mark_node and
   returns the semantic unchanged, with no diagnostic.  The assertion then
   runs with the build's default semantic instead of the label's.
   ...
   object; so does a label with no address-needing facet,
   ...
   The string facets escape this because grok_contract applies them
