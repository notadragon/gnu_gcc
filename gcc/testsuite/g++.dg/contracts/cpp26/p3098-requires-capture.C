...
// (see P4283 overview).
//
// The combination is easy to misparse: a requires-clause constraint parse that
// greedily consumes the following `[captures]` as a subscript
// expression leaves the capture name "not declared" and the result-name
// introducer misparsed.  So in a contract requires-clause, a `[` after the
