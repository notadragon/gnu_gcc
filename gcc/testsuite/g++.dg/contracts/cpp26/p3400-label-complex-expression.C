/* Test a P3400 assertion-control specifier whose expression contains
   ...
   type is followed by a contract specifier.  Its "< ... >" skip has to know
   about (), [] and {} nesting, and must not bail
   outright on a semicolon.  Counting angle brackets alone makes a label
   expression containing a relational
   operator inside parentheses, or a lambda body containing a statement,
   stop the skip in the wrong place; the recognizer then declines the
   contract, the tentative abstract-declarator parse swallows "pre<...>",
   and the whole declaration fails with "expected initializer".
