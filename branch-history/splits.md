...
Three statements in one insertion, belonging to two commits, and they
interleave -- which is why a cut is needed rather than two selectors.

`0160-requires-clause-on-parameter` owns the classification call and the
requires-clause check; `1250-contract-on-non-function-declarator` owns the
contracts check that sits between them.  The order is the order the two
specifiers are refused in the source, not the order the commits land, so the
contracts commit takes the middle piece and the requires commit takes the
two around it.

`nf_kind` travels with piece 0 deliberately: the requires commit comes first
and cannot reference a variable the contracts commit would introduce.

- +/^  \/\* Refuse a contract on a declarator that will not become a function/
- +/^  \/\* A requires-clause is refused in the same places/
