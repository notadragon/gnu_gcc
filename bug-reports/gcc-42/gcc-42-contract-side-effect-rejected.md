...
**Affects:** re-measured 2026-09-14 -- rejected on stock trunk
(17.0.0 20260914, built at `b0730e1d39f`) in all seven predicate shapes;
...
note below.
**Resolved by:** [d81fd685f92e](https://github.com/notadragon/gnu_gcc/commit/d81fd685f92e368a66d65e74c896553311c9d537)
...
Root cause: contract predicates are evaluated under `modifiable_tracker`
...
Suggested fix: distinguish those two outcomes rather than removing the
tracker.  Record, on the refusal path, that a store was refused and to which
object.  When a contract predicate's evaluation fails and a store was
refused, re-run the predicate without the tracker and let the modification
stand -- the program is well formed and required to succeed.  Because the
result then depends on which evaluation semantic was chosen (under `ignore`
the predicate is not evaluated at all), warn rather than error, which is
what the standard's ODR note is actually cautioning about.
...
This branch implements the suggested fix in [d81fd685f92e](https://github.com/notadragon/gnu_gcc/commit/d81fd685f92e368a66d65e74c896553311c9d537),
...
([a095a39df8b2](https://github.com/notadragon/gnu_gcc/commit/a095a39df8b2a1e15916e6779cfb7263889b7a8e) decides membership by liveness instead).
...
[a095a39df8b2](https://github.com/notadragon/gnu_gcc/commit/a095a39df8b2a1e15916e6779cfb7263889b7a8e), not by the fix recorded on this row.
...
because [a095a39df8b2](https://github.com/notadragon/gnu_gcc/commit/a095a39df8b2a1e15916e6779cfb7263889b7a8e) corrected the membership test before
