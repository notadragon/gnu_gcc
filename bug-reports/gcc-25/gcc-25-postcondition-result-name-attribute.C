// GCC-25 / PR125725: a postcondition's result-name-introducer does not
// accept `identifier attribute-specifier-seq :`.
//
...
// ... }` annotation, replaced with a plain `// expected-warning "..."`
// comment marking the line that must still produce a real
// [[deprecated]] warning (proof the attribute is applied, not just
// parsed and dropped). Requires -fcontracts
// -fcontract-evaluation-semantic=observe (C++26), linked with
...
int g (int x) post (r [[maybe_unused, deprecated]]: r > 0) { return x; } // expected-warning "'r' is deprecated"
