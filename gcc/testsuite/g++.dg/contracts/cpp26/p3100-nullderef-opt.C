...
// -O2 -- the level at which the failure manifests, since that is where the
// callee is inlined into main and a re-resolved reaction deletes the check.
// The other levels do not exercise the inlining, which is why this one is the
// load-bearing guard.
//
