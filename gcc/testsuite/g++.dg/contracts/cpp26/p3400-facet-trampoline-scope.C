/* Test that a P3400 facet label's trampolines can be generated from every
   ...
   grokked, so they must save the class scope as well as cfun.  Saving cfun
   alone lets finish_function's unconditional
   "if (current_class_name) ... pop_nested_class ()" pop a
   class binding level that start_preparsed_function never pushed (the
   trampoline's DECL_CONTEXT is the global namespace), tripping the
   binding-level assertion in poplevel_class.  Every shape below reaches it.
