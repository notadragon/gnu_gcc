...
//
// The hazard on the virtual interface path is a silently swallowed exception:
// the wrapper emits no capture initializer if its
// shared capture VAR_DECL has DECL_INITIAL destructively cleared by the base
// function's emission before the wrapper is copied.  The initializer is
// therefore preserved and restored for the wrapper.
