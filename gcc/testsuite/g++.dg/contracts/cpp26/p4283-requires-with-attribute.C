...
// [[attr]] (predicate)`).  This combination is where a TrailingObjects layout
// is easy to get wrong -- a requires-clause slot shifting the
...
// warning) rather than stored -- so there is no analogous defect here; this
// confirms the combination is accepted at all.
