/* Test that one label type yields one facet trampoline no matter how many
   ...
   tree per instantiation for one and the same type, so a cache keyed on the
   type tree as written misses and emits a fresh trampoline for every
   instantiation.  Keying on TYPE_MAIN_VARIANT collapses them back to one.

   The count is checked with a dg-final scan rather than at run time because
   the duplication is pure code bloat -- the names are counter-unique and
   internal-linkage, so the program behaves correctly either way and nothing
...
/* One trampoline for the one label type, not one per instantiation.  The
   ...
   second trampoline appearing at all is the defect: a per-instantiation
   cache miss gives all four their own (_0 through _3).  */
