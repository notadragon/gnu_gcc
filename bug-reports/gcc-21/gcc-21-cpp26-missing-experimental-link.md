...
have a discrete fix commit: `aaeebd5f2e3` ("driver: link the contracts
runtime for -std=c++29 too"), which adds `OPT_std_c__29`/`OPT_std_gnu__29` to
...
support was complete from `97fd9dcfb24` onward) and the C++29 gap (a real,
separate one closed later by `aaeebd5f2e3`) are two genuinely separate
defects, not two halves of the same fix.
