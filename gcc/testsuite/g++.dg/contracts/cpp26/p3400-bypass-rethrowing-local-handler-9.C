...
// frontier markers, and a walk that treats an unrecognized statement as
// unanalysable silently stops firing in any debug
// build.  Every other test here compiles without
// -g, so nothing else in the suite can see it: the symptom is identical code
// with and without the bypass, and no diagnostic either way.
//
