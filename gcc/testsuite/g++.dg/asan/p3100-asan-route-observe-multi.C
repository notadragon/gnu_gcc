...
// ASAN_OPTIONS).  Two env-driven gates stand between a stock ASan report and
// the handler, and routed noexcept_observe has to ignore both:
//   * the SECOND violation at a DIFFERENT site is dropped if the first report
//     consumes the process-wide crash-state latch, so the second hits the
//     `halt_on_error_ && !__sanitizer_acquire_crash_state()` gate and
//     early-returns without invoking the handler;
//   * whether a same-site repeat re-fires depends on suppress_equal_pcs.
// The routed path consults neither: every distinct
