/* A lambda's contract specifier was never substituted when the lambda was
   ...
   Nesting and generic lambdas are NOT part of the trigger, though they are
   easy to mistake for it: on a
   release build the postcondition face is compiled out and looks like
...
/* A lambda nested inside a generic lambda: the shape the bug is reported as,
   kept so it stays covered.  */
static int
...
/* NOT COVERED HERE: a predicate naming a CAPTURE rather than a parameter,
   ...
   -- which is a separate defect: the pattern's capture proxy
   has no local specialization either, and substitution reaches the same
   recovery path.  It segfaults on stock 16.2.0 and on trunk, independently of
   the substitution above, which neither causes nor cures it.  The non-template
