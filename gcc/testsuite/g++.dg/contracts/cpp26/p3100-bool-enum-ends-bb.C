...
//
// Regression test: bailing out of instrument_bool_enum_load_contract on
// stmt_ends_bb_p, before the configured reaction is consulted,
// leaves the raw load in place -- that is, behaves as assume no matter
// what semantic was configured, including the defined-value substitution
// that ignore requires.  The stock instrument_bool_enum_load handles the
// case, by retargeting the load and inserting on the
// fallthrough edge.
//
...
/* One per function -- the block-ending shapes as well as the two plain
   forms.  */
