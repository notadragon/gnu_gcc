...
(open, `confirmed`) -- linked the same way from CLANG-9 in the companion
Clang fork,
[notadragon/llvm-project](https://github.com/notadragon/llvm-project). Filing
...
Clang's counterpart is CLANG-1 in the companion Clang fork, whose row
(a) has already been fixed there -- that fix does not close rows (b) or (c)
on Clang either.

## Notes

Scope decision, 2026-09-04: report the full gap (rows (b)
...
check. Tracked as CLANG-9 in the companion Clang fork.

Why GCC does not catch (a): the most-derived type is statically known here,
so GCC folds the derived-to-virtual-base conversion to a constant offset and
the genericized tree is a plain `COMPONENT_REF` chain, with no vtable or VTT
lookup at all. Nothing consults the object, so there is no dynamic-type
operation on which a liveness check could hang -- which is exactly where
Clang's own check sits. The header comment of
[`gcc-02-constexpr-vbase-before-ctor.cpp`](gcc-02-constexpr-vbase-before-ctor.cpp)
spells this out alongside the reproducer.
