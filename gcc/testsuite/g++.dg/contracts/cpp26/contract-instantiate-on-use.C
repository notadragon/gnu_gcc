...
//
// -- and without this substitution each of them walks the still-dependent
// predicate straight into an ICE (`in check_noexcept_r, at cp/except.cc:1063`,
// or `in dependent_type_p, at cp/pt.cc:30282` for a type-dependent predicate).
// [dcl.contract.func]/9 puts the odr-use
// and the definition on the same footing; maybe_instantiate_contracts is the
// odr-use half.
//
// `pre (true)` does not exercise this: a predicate that is already
// a constant needs no substitution to be usable, so it passes either way.
// Every predicate below contains something
...
// Control: a predicate that is already a constant needs no substitution at
// all.  Here so that narrowing the substitution to
