...
// violation handler under noexcept_enforce must TERMINATE the program, not escape into the
// sanitizer runtime's non-unwindable C frames.  Routes the UBSan float-divide-by-zero check
...
// 7) and that no "ESCAPED"/"survived" text is produced.
//
...
// { dg-do run }
// { dg-additional-options "-std=c++26 -fcontracts-p3100 -fcontracts-p4298 -fsanitize=float-divide-by-zero -fsanitize-semantic=float-divide-by-zero:noexcept_enforce" }
// { dg-skip-if "requires hosted libstdc++ for stdc++exp" { ! hostedlib } }
// { dg-shouldfail "throwing handler under noexcept_enforce terminates" }
