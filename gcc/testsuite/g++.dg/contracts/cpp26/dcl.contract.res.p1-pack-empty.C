...
/* The same shapes written as an abbreviated function template, where the
   pack is invented from `auto...' rather than declared.  Nothing about the
   walk distinguishes an invented template parameter pack from a written one,
   but this is the spelling the ICE has repeatedly been reported in, so it is
   pinned down here too.  */
void abbrev_pre (auto... a)
  pre (true)
{ }

int abbrev_front_const (const auto x, auto... a)
  post (r : r > x)
{ return x + 1; }

int abbrev_front_nonconst (auto x, auto... a)	// { dg-error "value parameter 'x' used in a postcondition must be const" }
  post (r : r > x)				// { dg-error "a value parameter used in a postcondition must be const" }
{ return x + 1; }
