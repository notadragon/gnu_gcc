/* Test _Pre on an old-style (K&R) function definition.  Regression test:
   ...
   non-defining-declaration cleanup must not truncate those globals
   unconditionally -- doing so silently discards the enclosing function's own
   not-yet-injected contract before it is ever checked.  */
/* { dg-do run } */
