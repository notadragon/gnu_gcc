...
`contract-retval-destroyed-on-unwind-coro.C` (the same count for a coroutine
ramp), `postcondition-throw-escapes-try.C`,
`postcondition-throw-noexcept-terminate.C`, and
`open-bug-retval-throwing-cleanup.C`.

## Coroutines reach this too, and a sentinel-based fix will not reach them

Measured 2026-09-18 on stock trunk (17.0.0 20260909): a coroutine whose ramp
carries a postcondition leaks its return object by exactly one when the
violation handler throws -- `live == 1` where the non-coroutine case gives
the same answer for the same reason.  So the defect is not specific to
ordinary functions; the ramp is another path into it.

This matters for how it is fixed upstream, because the obvious fix does not
work there.  A coroutine ramp has no `current_retval_sentinel` to hang a
cleanup on: `cp_coroutine_transform::apply_transforms` clears
`cp_function_chain->throwing_cleanup` (`coroutines.cc`, "we must manage the
cleanups ourselves") and runs BEFORE `maybe_apply_function_contracts`, so
`maybe_set_retval_sentinel` never builds one and
`maybe_splice_retval_cleanup` takes its "only using the sentinel for an NRV"
exit.  Anything built on the sentinel therefore fixes ordinary functions and
silently leaves coroutines leaking.

Two ways out, neither free: give the checks their own unguarded cleanup for
the ramp -- what this branch does, since the region is reached only on the
body's normal-completion path where the returned object necessarily exists --
or teach the coroutine transform to bring the postcondition checks inside the
ramp's own cleanup.  A submission should say which it chose and why, because
a reviewer looking only at the ordinary case will not see the question.
