...
_classify() {
    ...
    # "confused by earlier errors, bailing out" IS an internal compiler error.
    # diagnostics/context.cc prints it in place of the ICE report when the
    # compiler was built without checking and an error has already been
    # emitted, and that branch is reachable from nowhere else.  Our branch
    # compiler is a Release build and the stock nightly is a checking build,
    # so without this line the same defect reads `ice` in one column and
    # `error` in the other -- and a row whose branch column already says
    # `error` can never record its own repair, which is most of what this
    # script is for.  Found on GCC-49, where it had quietly cost a row.
    if grep -qE 'internal compiler error|Please submit a full bug report' "$filtered" \
	    || grep -q 'confused by earlier errors' "$filtered"; then
        klass=ice
