// P3100 x P4298: misaligned access configured to "noexcept_enforce": reports
// (sem=7) then terminates.
...
// { dg-shouldfail "misaligned noexcept_enforce reports then terminates" }
#include <contracts>
#include <cstdio>
...
// { dg-output "VIOL kind=7 sem=7 comment=\\\[misaligned pointer access\\\]" }
