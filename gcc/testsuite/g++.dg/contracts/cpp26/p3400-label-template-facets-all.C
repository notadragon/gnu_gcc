/* Test that a DEPENDENT P3400 label's remaining facets take effect at
   ...
   Regression test: label substitution in tsubst_contract covers
   compute_semantic, allowed_semantics, local_violation_label and
   queryable_label.  Two more facets need explicit work there, or they are
   silently dropped:
     ...
     - compute_comment / compute_message.  apply_label_string_facet is
       ...
       on a dependent label, so without re-application at substitution the
       comment stays the raw predicate text.

     - group_names.  Worse than "never ran": caller-side resolution runs
       at the call site, before the definition is instantiated, so
       ensure_contract_groups sees the still-dependent pattern label and
       caches error_mark_node in CONTRACT_GROUPS.  copy_node in
       tsubst_contract then carries that poison onto the instantiation,
       where it makes every later group lookup early-out -- so a
       group-based configuration silently does not apply.

   The label has to be DEPENDENT for either failure to show.  A label that is
   ...
   function template, so a test written with one of those passes whether or
   not substitution re-applies anything.  Each case therefore spells the
   label as a variable template indexed by T, with a non-template control
   alongside.  */
