/* A P3400 facet label written as a prvalue -- pre<L{}> -- must have its
   ...
   materialization must not pre-fold the expression with cxx_constant_value
   and pass the result to pushdecl_top_level_and_finish: a fold that
   returns error_mark_node skips the whole thing in silence,
   leaving CONTRACT_LABEL a prvalue.  build_contract_check requires a
   VAR_DECL there, so the local-violation handler is then never wired up and
   the facet simply does not run -- no diagnostic, the contract just
   behaves as if unlabelled.

   So the unfolded expression goes to cp_finish_decl, which does
   the evaluation, and the decl used is the one pushdecl_namespace_level
