...
//
// The hazard is a compile failure, for every capture shape:
//
...
// post_capture violation (P3098's phase model); a capture site -- the
// ordinary one or the virtual capture struct -- that calls begin_handler
// unconditionally makes doing_eh() in cp/except.cc error out when
// flag_exceptions is off.  With no exceptions nothing can throw, so the
// try/catch is not built at all.
//
