/* Test that defining a GNU nested function does not discard the enclosing
   ...
   Regression test: the nested-function finish-function branches must not
   clear active_postconditions wholesale.  A nested function finishes while
   its enclosing function is still being parsed and still has its own
   entry live, so clearing the lot leaves every return site after the nested
   definition -- and the void implicit-return path -- seeing no active
   postcondition and getting no check, silently and with no diagnostic.

   The nested functions below deliberately have no contracts of their own:
   merely defining one is enough to reach this.

   Note contracts-nested-post.c cannot catch this -- it only
