...
//
// A pure virtual called by unqualified
...
// assertions aren't needed enough" -- proposes a bullet
// matching [except.spec]'s "in an expression, the function is selected by
// overload resolution", narrowed to potentially evaluated expressions.  GCC
// reaches the same place from maybe_instantiate_contracts rather than through
// odr-use, so the wording gap is not observable here.
//
