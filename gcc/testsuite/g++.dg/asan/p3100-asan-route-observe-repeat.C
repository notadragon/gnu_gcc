...
// using a fixed 25-entry pool (kAsanBuggyPcPoolSize).  Deferring to it once
// that pool is full returns "suppress" for everything, so every
// subsequent routed violation anywhere in the process is silently
// dropped -- including at sites never reported once.  With
// 30 distinct sites that yields 25 handler calls, not 30.
//
