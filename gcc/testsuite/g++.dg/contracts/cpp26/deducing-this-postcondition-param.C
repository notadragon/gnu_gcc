...
// Upstream's contracts testsuite covers nothing about deducing this.
//
...
// reported it twice -- but only becomes visible on a declared-only one once
// such a template's contracts substitute at its odr-use.  Tracked as GCC-36
// in bug-reports/; when that is fixed one of these two expectations goes away.
