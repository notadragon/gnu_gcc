...
// REQUIRES_EXPR_REQS.  Guarding only the first group leaves a parameter
// reached through a decltype or a requires-expression wrongly marked
// odr-used -- rejecting programs stock g++ accepts, and, because the same
// flag drives the coroutine rule, telling a coroutine its parameter is
// odr-used when it is not.
//
