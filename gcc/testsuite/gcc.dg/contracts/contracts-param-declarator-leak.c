/* A contract written on a parameter's own function declarator must not be
   ...
   nested inside a parameter list.  If the enclosing function has no
   contract of its own, nothing truncates the globals, and the
   parameter's contract is injected into the enclosing function body.
   That is not a lost contract but a wrong one: `_Pre (0)` firing against a
   function that never had a precondition.

   C++ accepts and ignores the same construct, so ignoring is the right
   behaviour; it is warned about rather than dropped silently.  */
